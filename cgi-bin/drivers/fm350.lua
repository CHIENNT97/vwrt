...
local function acquire_lock()
    ...
    os.execute("rm -rf " .. LOCK_DIR .. " && mkdir " .. LOCK_DIR .. " 2>/dev/null")
    return true
end
...
local function get_qmodem_port()
    local raw = exec("ubus call modem_ctrl info '{}' 2>/dev/null")
    if not raw or raw == "" then return nil end
    local parsed = nil
    pcall(function() parsed = json.parse(raw) end)
    if not parsed or not parsed.info or not parsed.info[1] or not parsed.info[1].modem_info then return nil end
    for _, item in ipairs(parsed.info[1].modem_info) do
        if item.key then
            local norm = item.key:lower():gsub("[%s_]+", "")
            if norm == "atport" and item.value and item.value ~= "" then
                return item.value
            end
        end
    end
...
local function get_fm350_port(purpose)
    -- Check QModem first
    local q_port = get_qmodem_port()
    if q_port then return q_port end
